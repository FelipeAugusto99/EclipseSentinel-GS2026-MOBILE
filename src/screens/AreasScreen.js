...
import { areas as dadosIniciais } from '../mock';
 
export default function AreasScreen() {
  const [areas, setAreas] = useState(dadosIniciais);
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        data={areas}
        keyExtractor={item => String(item.id_area)}
        renderItem={({ item }) => (
          <View style={styles.card}>
            <View style={styles.cardInfo}>
              <Text style={styles.cardTitulo}>{item.nome_area}</Text>
              <Text style={[styles.badge, { color: corRisco(item.nivel_risco) }]}>
                {item.nivel_risco}
              </Text>
              <Text style={styles.cardStatus}>{item.status_area}</Text>
            </View>
            <View style={styles.cardAcoes}>
              <TouchableOpacity onPress={() => abrirEditar(item)}>
                <Text style={styles.botaoEditar}>Editar</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => excluir(item.id_area)}>
